...
export function registerGscTools(server: McpServer) {
  ...
  server.tool(
    ...
    "🚫 USE ONLY for URLs with JobPosting OR BroadcastEvent (live stream) structured data. Google's Indexing API is HARD-RESTRICTED to those two schema types — confirmed by official docs (April 2026) and Google has been tightening enforcement (now requires partner authorization for high-volume access). For ALL other URLs (blog posts, landing pages, program pages, sedes), this call returns 200 OK but Google silently no-ops it (verified live: status check after submit returns 404 because no metadata is registered). The GSC UI 'Request Indexing' button is NOT exposed as an API; reverse-engineering it violates ToS. The sitemap ping endpoint (google.com/ping) was deprecated in June 2023. Google has NOT adopted IndexNow as of 2026. For regular content, the LEGITIMATE path is: (1) gsc_sitemaps_submit to force a sitemap re-fetch, (2) ensure accurate <lastmod> dates in your sitemap, (3) use gsc_url_bulk_inspection to verify coverage post-deploy. Or run the seo_workflow_playbook with name='request_recrawl' for the full recipe.",
    {
